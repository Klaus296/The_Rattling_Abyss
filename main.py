...
pygame.display.set_caption("The Rattling Abyss")
class Area:
...
class Hero(Picture):
    def __init__(self, frames, x=0, y=0, width=50, height=50):
        ...
        self.base_height, self.min_height = 110, 95
...
l = 0
def arrows_attack():
    global arrows, hero, health_rect, frame, background, ladder,l,k, keys, ladder2, frame_lose
...
def cor():
    global way,hero, k, frame,warNolar_health, temarius_health,set_run
    frame = 0
    hero.speed = 5
    set_run = True
    way.show()
    warNolar_health.rect.width = 200
    temarius_health.rect.width = 200
...
plot_list = [lambda:start(),lambda:show_text(first_text, 10, 10, font_size=30, show_items=[farian]),lambda:end(),lambda:timer(50),
             ...
             lambda:show_text(before_first_fight,10,10,font_size=30, show_items=[seijiro]),lambda:timer(50),
             ...
             lambda:start(),lambda:show_text(after_temarius,10,10,font_size=30,show_items=[farian]),lambda:timer(90),lambda:the_end(),lambda:the_end_2()
            ]
k = 0
def plot():
